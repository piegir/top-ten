...
export let getRoundPlayers = () => { return makeGetCall("/rounds/get_players");}

function getCurrentPlayer() { return makeGetCall("/rounds/get_current_player");}
...
export class ThemeSelection extends Component {
  ...
  <div className = "GameProgressBox"><GameProgress /><
      /div>
                </div><div className = "BottomBox"><
      Users getUsersListHandler = {getRoundPlayers} checkOnlyOnce =
          {true} displayNumbers =
              {false} />
                    {this.state.firstRoundPlayer === currentUser.username ?
                      ...
                      () => {
                        getTheme().then((theme) => {
                          this.setState({
                            ...
                            currentPlayer : this.state.currentPlayer,
                            allPropositionsMade :
                                this.state.allPropositionsMade,
                          });
                          if (theme === null) {
                            // Get theme until it's not null
                            this.currentThemeGettingId =
                                repeat(this.getCurrentTheme, 100);
                          }
                        });
                      }
                          ...
                          checkAllPropositionsMade().then(
                              (allPropositionsMade) => {
                                ...
                                if (allPropositionsMade) {
                                  ...
                                  this.turnStatusCheckingId =
                                      repeat(this.checkTurnStatus, 100);
                                }
                              });
                        });
                      }
        ...
        return (
            <div className="GlobalGrid">
                ...
                <div className="BottomBox">
                    ...
                    {this.state.firstRoundPlayer === currentUser.username ?
                        <MakeHypothesis goToRoundResultsCheckingHandler={
