...
export class MakeHypothesis extends Component {
  ...
  lower = (index) => {
    [this.state.hypothesis[index + 1], this.state.hypothesis[index]] =
        [ this.state.hypothesis[index], this.state.hypothesis[index + 1] ];
    this.setState({
  ...
  dragStarted = (event) => { this.row = event.target; }
  ...
  render() {
    return (
        <div className = "UserActionBox"><div className = "SubTitle">Make your hypothesis by dragging rows</div>
            ...
            /th>
                    </tr>{this.state.propositions
                                               .map((proposition) => {
                                                 let propHypIndex =
                                                     this.state.hypothesis
                                                         .indexOf(proposition);
                                                 return (
                                                     <tr draggable = {true} onDragStart =
                                                          {this.dragStarted} onDragOver =
                                                              {this.dragOver} style = {
                                                                {
                                                                  cursor:
                                                                      "all-scroll",
                                    ...
                                    <td>
                                        {proposition.proposition}
                                                     ...
                                                     <
                                                         div className =
                                                             "ButtonBox"><button onClick = {this.makeHypothesisHandler}>Submit</button>
                </div></div>
        );
    }
}
