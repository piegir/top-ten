...
export function getColorFromScale({
  ...
  minValue = 0,
  maxValue = 100,
  beginColor = colors.darkGreen,
  endColor = colors.red,
  middleColor = colors.nicerYellow,
  opacity = 1
}) {
  return getColorFromScaleValues(value, minValue, maxValue, beginColor,
                                 endColor, middleColor, opacity);
}

let getColorFromScaleValues =
    (value, minValue, maxValue, beginColor, endColor, middleColor, opacity) => {
      let middleValue = (maxValue - minValue) / 2;
      let arrayColor;
      if (value < middleValue) {
        arrayColor =
            interpolateColor((value - minValue) / (middleValue - minValue),
                             beginColor, middleColor);
      } else {
        arrayColor =
            interpolateColor((value - middleValue) / (maxValue - middleValue),
                             middleColor, endColor);
      }
      return `rgba(${arrayColor[0]}, ${arrayColor[1]}, ${arrayColor[2]}, ${
          opacity})`;
    }
...
export function makeGetCall(
    endpoint) { return makeRestApiCall(endpoint, endpointTypes.GET); }
